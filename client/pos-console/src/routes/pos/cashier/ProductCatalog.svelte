<script lang="ts">
	...
	// load data from backend API（替代 Dexie 数据源）
	import { getCategories, getProducts, type UiCategory as Category, type UiProduct as Product } from '$lib/catalog';
	...
	onMount(async () => {
		...
		try {
			categories = [defaultCategory, ...(await getCategories())];
