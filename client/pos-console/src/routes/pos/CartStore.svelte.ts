...
import { cashier } from '$lib/session.svelte';
...
export class CartStore {
	...
	// 真实下单：POST /orders（金额服务端重算；幂等键防重复扣款）。返回是否成功。
	checkout = async (method = 'cash'): Promise<boolean> => {
		if (!cashier.loggedIn || cashier.warehouseId == null) {
			toast.error(m.pos_need_login());
			return false;
		}
		const items = this.cart.map((i) => ({ product_id: i.product.id, quantity: i.quantity }));
		const payments = method === 'cash' ? [] : [{ method, amount: this.grandTotal }];
		try {
			const order = await api.post<{ total: string }>('/orders', {
				warehouse_id: cashier.warehouseId,
				idempotency_key: crypto.randomUUID(),
				payments,
				items
			});
			toast.success(m.pos_checkout_success({ total: order.total }));
			this.clearCart();
			this.showCartOnMobile = false;
			return true;
		} catch (e) {
			toast.error(e instanceof ApiError ? e.message : m.pos_checkout_failed());
			return false;
		}
	};
